...
offset_of {ts,i} (pfin | tup, i) = let
  ...
  (
    ...
    in
      ...
      prval t0ype_in_t0ypes_succ {x} {b,b1} (pfin1) = pfin
...
tup_get<tup2 (A, b)><A><Z()> {ts} {i} (pfti, pfin | tup) = let
  prval TIEQZ() = pfti
  prval t0ype_in_t0ypes_zero () = pfin
in
  tup.0
end // end of [tup_get]
implement(B,b,A,N)
tup_get<tup2 (B,b)><A><S(N)> {ts} {i} (pfti, pfin | tup) = let
  ...
  prval t0ype_in_t0ypes_succ {x} {t,t'} (pfin) = pfin
...
//
prval pf_in0 = t0ype_in_t0ypes_zero ()
