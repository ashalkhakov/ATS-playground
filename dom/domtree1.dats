...
, (*self*)addr
  ...
  | {n,h:nat | h <= n}
    ...
    {slfp,slfn,slfc:addr}
    {pnt:addr} {dir,ntot:nat | dir <= ntot} {rt,rtp:addr}
    {slf:agz;slfp,slfn:addr}
    DIFFnode (n, hs, lm, lmp, lmn, nf, nf+nr+1, rt, rtp, slf) of (
    ...
    , domdiff_v (n, h, slf, slfp, slfn, dir, ntot, rt, rtp, pnt)
    ) // end of [DIFFnode]
...
domdiff_domtree_join
  ...
  {dir,ntot:nat} {rt,rtp,slf:addr} .<n-h>. (
    fpf: domdiff_v (n, h, chld, chldp, chldn, dir, ntot, rt, rtp, slf)
...
prfun
domdiff_takeout
  {n,h:int} {chld,chldp,chldn:addr} {dir,ntot:int} {rt,rtp:addr}
  {slf:addr | slf <> rtp} .< >. (
    pf: domdiff_v (n, h, chld, chldp, chldn, dir, ntot, rt, rtp, slf)
  ) :<> [h0:int;pnt,slfp,slfn,lc:addr] (
    domnode_v (h0, pnt, slfp, slfn, lc, slf)
  , domnode_v (h0, pnt, slfp, slfn, lc, slf)
      -<lin,prf> domdiff_v (n, h, chld, chldp, chldn, dir, ntot, rt, rtp, slf)
  ) = let
...
, int(*total nr of parent's children*)
, addr(*root*)
, addr(*parent of root*)
...
vtypedef domtraversal_vt0 =
  domtraversal_vt (0, 0, 0, 0, null, null, null, null)?
...
domtree_traversal_beg {h:nat} {rt:agz} (
  domtree_v_root (h, rt)
| p: ptr rt
, tr: &domtraversal_vt0 >> domtraversal_vt (h, h, 0, ntot, rt, null, slf, chld)
): #[slf,chld:addr;ntot:nat] void
...
extern
fun
domtree_traversal_end {n,h:nat | h <= n} {dir,ntot:int} {slf,chld,rt:addr} (
  &domtraversal_vt (n, h, dir, ntot, rt, null, slf, chld) >> domtraversal_vt0
): (
  domtree_v_root (n, rt)
| ptr rt
...
domtree_traversal_child {n,h,dir,ntot:int} {rt,rtp,slf:addr;chld:addr} (
  &domtraversal_vt (n, h, dir, ntot, rt, rtp, slf, chld)
    >> domtraversal_vt (n, h1, 0, ntot1, rt, rtp, chld, chld1)
): #[chld1:addr] #[h1:nat | h1 <= h] #[ntot1:int] void
//
// moves up to the parent node
extern
fun
domtree_traversal_parent {n,h,dir,ntot:int} {rt,rtp,slf,chld:addr | slf <> rtp} (
  &domtraversal_vt (n, h, dir, ntot, rt, rtp, slf, chld)
    >> domtraversal_vt (n, h1, dir1, ntot1, rt, rtp, slf1, slf)
): #[dir1,ntot1,h1:nat | h <= h1; h1 <= n] #[slf1:addr] void
// checks if the focussed node has a parent
extern
fun
domtree_traversal_has_parent {n,h,dir,ntot:int} {rt,rtp,slf,chld:addr} (
  &domtraversal_vt (n, h, dir, ntot, rt, rtp, slf, chld)
): bool (slf <> rtp)
//
...
extern
fun
domtree_traversal_next {n,h,dir,ntot:int | dir < ntot} {rt,rtp,slf,chld:addr(*| slf <> rtp*)} (
  &domtraversal_vt (n, h, dir, ntot, rt, rtp, slf, chld)
    >> domtraversal_vt (n, h1, dir+1, ntot, rt, rtp, slf, chld1)
): #[h1:nat | h1 <= n] #[chld1:addr] void
// moves to the preceding sibling
extern
fun
domtree_traversal_prev {n,h,dir,ntot:int | dir > 0} {rt,rtp,slf,chld:addr(*| slf <> rtp*)} (
  &domtraversal_vt (n, h, dir, ntot, rt, rtp, slf, chld)
    >> domtraversal_vt (n, h1, dir-1, ntot, rt, rtp, slf, chld1)
...
dataview param_validity (int, int, int, int, addr) =
  {n,h:nat | h <= n} {dir,ntot:nat | dir <= ntot} {chld:agz}
  param_validity_v (n, h, dir, ntot, chld)
//
assume domtraversal_vt (n:int, h:int, dir:int, ntot:int, rt:addr, rtp:addr, slf:addr, chld:addr) = @{
  p_rtp= ptr rtp
, p_root= ptr rt
, p_slf= ptr slf
, p_chld= ptr chld
, pf_valid = param_validity (n, h, dir, ntot, chld)
, pf_ctx = [chldp,chldn:addr] (
    domdiff_v (n, h, chld, chldp, chldn, dir, ntot, rt, rtp, slf)
...
//
implement
domtree_traversal_end {n,h} {dir,ntot} {slf,chld,rt} (
  tr
) = let
...
//
implement
domtree_traversal_child {n,h,dir,ntot} {rt,rtp,slf,chld} (tr) = {
//
prval param_validity_v () = tr.pf_valid
...
//
implement
domtree_traversal_has_parent {n,h,dir,ntot} {rt,rtp,slf,chld} (tr) = let
//
prval param_validity_v () = tr.pf_valid
//
...
//
implement
domtree_traversal_parent {n,h,dir,ntot} {rt,rtp,slf,chld} (tr) = {
//
prval param_validity_v () = tr.pf_valid
prval (pf_diff, pf_tree) = tr.pf_ctx
//
...
//
implement
domtree_traversal_next {n,h,dir,ntot} {rt,rtp,slf,chld} (tr) = {
//
prval param_validity_v () = tr.pf_valid
prval (pf_diff, pf_tree) = tr.pf_ctx
//
...
//
implement
domtree_traversal_prev {n,h,dir,ntot} {rt,rtp,slf,chld} (tr) = {
//
prval param_validity_v () = tr.pf_valid
prval (pf_diff, pf_tree) = tr.pf_ctx
//
